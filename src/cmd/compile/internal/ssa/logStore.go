...
func rtcall(pos src.XPos, fn *obj.LSym, b *Block, mem, sp, fnArg *Value,
	...
	ACArgs = append(ACArgs, Param{Type: fnArg.Type, Offset: int32(off)})

	off += fnArg.Type.Size()

	offR := round(off, resultT.Alignment())
	ACResults = append(ACResults, Param{Type: resultT, Offset: int32(offR)})

	// Issue call
	mem = b.NewValue1A(pos, OpStaticCall, types.TypeMem, StaticAuxCall(fn, ACArgs, ACResults), mem)
