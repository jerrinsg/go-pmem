...
var (
	...
	TxLog2FnOffset  int64
	TxLog2FnStkSize int64
)
			...
			case OpStore, OpZero, OpMove:
				if needLog(v) {
					...
					if willAccessPmem(ptr) {
						...
						// tx.Log() call here
						mem = txLog2Call(pos, b, mem, sb, sp, ptr, txHandle)
						v.RemoveArg(2)
						v.AddArg(mem)
						...
						memThen := txLog2Call(pos, bThen, memIf, sb, sp, ptr, txHandle)
...
func txLog2Call(pos src.XPos, b *Block, mem, sb, sp, ptr, txHandle *Value) *Value {
	f := b.Func
	widthPtr := int64(types.Widthptr)
	byteptr := f.Config.Types.BytePtr
	elemType := ptr.Type.Elem()
	elemTypeAddr := f.Entry.NewValue1A(f.Entry.Pos, OpAddr, byteptr, elemType.Symbol(), sb)

	// TODO: Assumed signature of Log2 is:
	// func Log2(src, dst unsafe.Pointer, size uintptr) error

	// allocate memory for one of the args to Log2() method. Log2() assumes memory
	// for the dst will be created and passed to it by user.
	mem, retV := rtcall(pos, f.fe.Syslook("pnewobject"), b, mem, sp, elemTypeAddr, ptr.Type)
	...
	// }
	// If runtime's representation of itab changes, below calculation needs to change
	fnOffset := TxLog2FnOffset + 2*int64(widthPtr) + 8
	fn := b.NewValue1I(pos, OpOffPtr, f.Config.Types.UintptrPtr, fnOffset, txHandle)
	codeptr := b.NewValue2(pos, OpLoad, f.Config.Types.Uintptr, fn, mem)
	...
	// Store args to the method. For Log2(), first two args are unsafe pointers,
	// and third arg is size of type to be stored in transaction logs.
	// Store src
	...
	// Issue call to Log2() method
	mem = b.NewValue2(pos, OpInterCall, types.TypeMem, codeptr, mem)

	// This depends on the signature of Log2() method in the transaction interface
	// This is set by gc for simplicity
	mem.AuxInt = TxLog2FnStkSize
	return mem
}
